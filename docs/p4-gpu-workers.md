...
模型只在每次推理子进程中加载；成功、失败、超时或取消后子进程退出，释放权重、缓存和 CUDA 上下文。常驻消费者只管理 CPU 队列，不持有 CUDA 上下文。启动时的 GPU 自检也在短命子进程执行。与二维服务共卡时，领取新任务前通过 nvidia-smi 检查显存和利用率；已有占用超过 512MiB 或利用率超过 10% 时等待，仅有二维卸载后的小型 CUDA 上下文可以继续。此检查不能阻止其他服务随后同时启动，OOM 仍按失败报告，未宣称跨服务严格互斥。
